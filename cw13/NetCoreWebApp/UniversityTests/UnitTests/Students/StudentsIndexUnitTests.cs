...
namespace UniversityTests.UnitTests.Students
{
    ...
    public class StudentsIndexUnitTests
    {
        ...
        [Test]
        public void IndexMethod_NoStudents(){
            //AAA
             //Arrange
            var dbLayer = new Mock<IStudentsRepository>();
            dbLayer.Setup(d => d.GetStudents()).Returns(new List<Student>());
        ...
        public void IndexMethod_StudentsWithGradesAndWithoutGrades_Correct(){
            ...
            dbLayer.Setup(d => d.GetStudents()).Returns(new List<Student>()
            {
                new Student{IdStudent=1, FirstName="Cezary", LastName="Boguszewski", Address="Warszawa, Koszykowa 77"},
                new Student{IdStudent=2, FirstName="Bogumił", LastName="Biedrzycki", Address="Warszawa, Zeromskiego 10"}
            });
            dbLayer.Setup(grades => grades.GetGrades(1)).Returns(new List<Grade>()
            {
                new Grade{IdGrade=1, IdStudent=1, GradeValue=4, Subject="Analiza Matematyczna", SubjectType="Group 1"}
            });
